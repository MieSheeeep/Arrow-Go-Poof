...
开始界面和游戏界面共用提交者提供的工作桌图片资源，文件位于 `assets/menu-background.png`；箭头棋盘会始终居中放在桌垫内。箭头使用 Pygame 绘制的居中回旋镖形方向标记，透明格可淡淡透出底层关卡颜色。第一关 BEAD BALL 根据提交者提供的 `.px` 拼豆工程图案转换为固定网格。其余界面与关卡图案均由 Pygame 基础图形绘制。
