...
项目启动后先显示带装饰箭头棋盘的主界面，点击唯一的 `START GAME` 按钮进入第一关。游戏窗口为 1200×800，中心区域是带隐藏像素颜色的箭头图案；三关依次为 16×16 附魔金苹果、16×16 精灵球和 18×17 月亮 Hello Kitty。成功点击后，箭头沿方向飞出窗口；被阻挡时箭头移动到首个阻挡箭头、撞击变红并回退。顶部状态栏显示关卡、计时、三颗生命和暂停键；暂停后可继续、重开本关或返回主界面；通关页显示冻结的时间和星级。
...
建议发布博客时补充：碰撞变红截图、最终通关截图，以及一段展示三关切换的 GIF 或视频。
...
项目包含 ENCHANTED APPLE、BEAD BALL、MOON KITTY 三个关卡。每个关卡由二维网格和颜色网格表示：`U/D/L/R` 是四种方向箭头，`.` 是已露出的像素位置，`None` 是图案外背景。箭头清除后，原位置显示颜色网格中的拼豆颜色。三关均根据提交者提供的 `.px` 拼豆工程图案制作：附魔金苹果和精灵球为 16×16 固定网格，月亮 Hello Kitty 在设计阶段等比例压缩为 18×17；三个方向布局都通过固定种子生成、验证后复制为常量，因此每次启动布局相同，不会因随机生成而出现不可解关卡。
...
计时由 `Game` 的 `elapsed_seconds` 统一维护，只在 `PLAYING` 状态累加；进入开始页、通关页或失败页后自动冻结，重开、下一关和返回菜单都会清零。每关有固定的三星/二星时间阈值：零失误并在三星时限内为 3 星；一次失误内且在二星时限内为 2 星；其余通关记为 1 星。评分作为不可变通关结果保存，因此不会随着结果页面停留时间继续变化。
...
当前结果请在提交前以本机最新命令输出为准；自动化测试覆盖暂停状态、暂停时计时冻结、暂停菜单按钮路由和用户提供的 UI 素材打包。
...
| T09 | 计时开始、冻结与重开 | 仅对局计时；通关/失败后冻结；重开清零 | `Game.elapsed_seconds` 状态转换测试通过 | 通过 |
| T10 | 时间与失误星级结算 | 3/2/1 星规则正确 | 四种时间/失误组合的参数化测试通过 | 通过 |
