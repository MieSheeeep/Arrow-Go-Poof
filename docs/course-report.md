...
项目启动后先显示开始界面，点击 `START GAME` 进入第一关。游戏窗口为 1200×800，中心区域是带隐藏像素颜色的箭头图案。成功点击后，箭头沿方向飞出窗口；被阻挡时箭头移动到首个阻挡箭头、撞击变红并回退。
...
项目包含 TREE、FLOWER、SUN 三个关卡。每个关卡由二维网格和颜色网格表示：`U/D/L/R` 是四种方向箭头，`.` 是已露出的像素位置，`None` 是图案外背景。箭头清除后，原位置显示颜色网格中的树叶、树干、草地或花朵颜色。
...
| 关卡设计与可通关验证 | Codex | 生成树、花朵和太阳的网格数据与测试结构 | 人工调整颜色掩码，并加入顺序清除验证；每个关卡实际由测试逐步清空 |
...
当前结果：**101 passed**；编译检查通过；Pygame 主循环在 dummy SDL 环境下可以正常启动、绘制并响应退出事件。
