...
![开始界面（封面）](assets/runtime-start.png)
...
倒计时由 `Game` 的 `elapsed_seconds` 统一维护，只在 `PLAYING` 状态累加；进入开始页、通关页或失败页后自动冻结，暂停时同样停止，重开、下一关和返回菜单都会清零。HUD 显示的是 `remaining_seconds`（剩余时间），并用一条进度条配合颜色从平静过渡到紧急——剩余 25 秒内变黄、10 秒内变红并脉冲。每关有固定的三星/二星时间阈值：零失误并在三星时限内为 3 星；一次失误内且在二星时限内为 2 星；其余通关记为 1 星。评分作为不可变通关结果保存，因此不会随着结果页面停留时间继续变化；通关时由 `StarRevealAnimation` 让已获得的星星从空星位置逐颗弹出。
...
当前结果以本机最新命令输出为准：`157 passed`。自动化测试覆盖 Board 规则、四方向路径、三关可通关顺序、固定种子布局、飞出与碰撞动画、扣心反馈、倒计时与星级结算、开始/通关/失败/暂停/重开、鼠标事件路由和 UI 布局。
...
| T10 | 时间与失误星级结算 | 3/2/1 星规则正确，星星逐颗弹出 | 时间/失误组合的参数化结算与 `StarRevealAnimation` 测试通过 | 通过 |
...
分层设计也帮了很大忙。把 Board 的规则、Game 的状态和 UI 的渲染拆开后，动画的时间模型可以脱离 Pygame 单独测试，倒计时、扣心、星星结算这些逻辑也都能用纯 Python 的单元测试覆盖，最终跑出 157 个通过的用例。没有这层边界，很多逻辑会散落在绘制代码里，既难改也难测。
